...
def load_coordinates_from_db(db_path,
                             coordinate_systems,
                             feature_cols=None,   # now accepts multiple feature columns
                             filter_values=None):
...
def efficient_spatialite_export(db_path,
    ...
    try:
        ...
        # feature filters (same logic as load_coordinates_from_db)
        if filter_values:
            for col, b in filter_values.items():
                if b.get("min") is not None:
                    conds.append(f"f.{col} >= ?")
        ...
        if pd.api.types.is_numeric_dtype(df[color_col]):
            ...
            if color_palette == "Category10":
                if len(categories) <= 10:
                    ...
                    palette = glasbey[:len(categories)]   # fallback if >10 categories
            elif color_palette == "glasbey":
                palette = glasbey[:len(categories)]
            else:
                palette = glasbey[:len(categories)]       # safe fallback
            ...
            [
                ...
                dbc.CardBody(
                    [
                        ...
                        #    ],
                        #    style={
                        #        "flex": "0 0 auto", #do not force shrink the box
                        #        "overflowY": "visible", # auto size to plot area
                        #        #"maxHeight": "calc(100vh - " + BANNER_HEIGHT + " - 250px)",
                        #        # 250px leaves room for controls/footer
                        #        "height": "auto",
                        #        "minHeight": "fit-content", # always fit all plots
                        #    }
                        #),
